...
// Helper to get Bangladesh (UTC+6) start of day/month
...
// Get current month invoice count for a shop (BD Time)
async function getMonthlyInvoiceCount(shopId) {
  const nowBD = getBDTime();
  // Get start of month in BD: 00:00 on the 1st
  const startBD = new Date(Date.UTC(nowBD.getUTCFullYear(), nowBD.getUTCMonth(), 1, 0, 0, 0, 0));
  // Convert back to UTC for DB query: 00:00 BD is 18:00 previous day UTC
  const startUTC = new Date(startBD.getTime() - 6 * 60 * 60 * 1000);
  return Invoice.countDocuments({ shopId, createdAt: { $gte: startUTC }, is_deleted: false });
}
...
exports.listInvoices = catchAsyncError(async (req, res, next) => {
  ...
  if (date) {
    // Bangladesh Time (UTC+6) range for the selected date
    const startBD = new Date(`${date}T00:00:00.000+06:00`);
    const endUTC = new Date(`${date}T23:59:59.999Z`);

    // Fetch all invoices and filter in JavaScript.
    // MongoDB date queries fail when invoiceDate is stored as a string,
    // but Mongoose auto-casts to Date when reading, so JS comparison works.
  ...
  // No date filter — use normal MongoDB query with pagination
  const skip = (pageNum - 1) * limitNum;
